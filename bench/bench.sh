...
###########################################################
# variables
BENCH_DIR="results-$(date +%Y-%m-%d)-numa"
ITERATIONS=4000000
PROGRSS_BAR=
MEMSET=
PARALLEL=true
PARALLEL_J=-j4
FUNCS="MALLOC FREE FULLOPS"
...
function gen_gnuplot_lines_commands()
{
	...
	for alloc in ./allocs/*
	do
		local alloc_name="$(basename ${alloc} | cut -f 1 -d '.' | sed -e 's/^lib//g' | sed -e 's/_/\\_/g')"
...
function gen_gnuplot_bar_commands()
{
	...
	do
		local alloc_name="$(basename ${alloc} | cut -f 1 -d '.' | sed -e 's/^lib//g' | sed -e 's/_/\\_/g')"
		echo "	'<cat $(gen_file_name ${size_mode} ${mode} ${alloc_name} ${sizes}) | grep ${func} | sort -n -k 2' u 5:4:6${ticks} title '${alloc_name}' ,\\"
		ticks=""
	done
	local alloc_name="default"
...
function gen_gnuplot_bar_2_commands()
{
	...
	do
		...
		do
			...
			printf "$(cat "${BENCH_DIR}/result-${size_mode}-size-${mode}-${size}-${alloc_name}.dat" | grep "${filter}")\t${alloc_name_escaped}\n"
		done > ${fname}
	done
